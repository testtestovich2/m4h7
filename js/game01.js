...
const setNumber = () => { // загадываем число
...
const tryToGuess = () => { // функция попытайся угадать что за число
    let message = 'Угадайте число от 1 до 100 за 10 попыток';
    ...
    while (attempts>0) { //условие провекри если попыток > 0
        let entered = prompt(message);//считываем то, что вводим в строке
        if (entered == null) { //если значение нулевое или пустое
            ...
            if (isNaN(entered) || entered.indexOf(' ')>=0 || entered=="") { // если ничего не введено или 0 
                ...
                let con = confirm("Правильно! Сыграем ещё раз?") // выбор при true
        ...
        if (attempts == 0) {  // колличество попыток равно 0 
            let con = confirm('Вы не угадали :( Загаданное число: '+targetNumber+'. Сыграем ещё раз?'); //какое число было
